...
agg as (
  select
    order_id,
    -- timestamps: take MIN to be deterministic
...
finalized as (
  select
    ...
    -- effective VAT % at order level: tax / net_before_tax * 100
    cast(
      case
...
select
  ...
  -- keys
  order_id,
  customer_id,                    -- ✅ now present

  -- timestamps
  created_at_local,
  created_at_utc,
  to_date(created_at_utc)  as order_date,        -- ✅ added for tests
